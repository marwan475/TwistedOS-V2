...
FileSystem::FileSystem(EFI_HANDLE ImageHandle, EFI_BOOT_SERVICES *BootServices,
                       Console *efiConsole)
    : ImageHandle(ImageHandle), BootServices(BootServices),
...
EFI_STATUS FileSystem::SetDirectoryPosition(EFI_FILE_PROTOCOL *Dir,
  ...
  if (FileInfo.Attribute & EFI_FILE_DIRECTORY) {
    ...
    status =
        Dir->Open(Dir, &KernelFile, FileInfo.FileName, EFI_FILE_MODE_READ, 0);
    ...
    status =
        BootServices->AllocatePool(EfiLoaderData, KernelSize, &KernelBuffer);
...
EFI_STATUS FileSystem::ReadFileSystem() {
  ...
  status = BootServices->OpenProtocol(
      ImageHandle, &LoadImageGUID, (VOID **)&LoadImageProtocol, ImageHandle,
  ...
  status = BootServices->OpenProtocol(
      LoadImageProtocol->DeviceHandle, &SimpleFileGUID,
